...
public:
	...
	virtual void onAnswered(long chatId, bool accepted, const std::string& errmsg) {
		if (accepted) {
			...
			LoggerWrapper::instance()->error("onAnswered: peer rejected, error message is %s", errmsg.c_str());
		}
	}

	virtual void onClosed(long chatId, const std::string& errmsg) {
		LoggerWrapper::instance()->info("onClosed: chatId is %ld, error message is %s", chatId, errmsg.c_str());
