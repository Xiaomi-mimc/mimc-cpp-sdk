...
int main() {
    ...
    mimc_set_test_packet_loss(user1, packetLossRate);


    while ((time(NULL) - user1_login_time < 5 && !mimc_isonline(user1))) {
        printf("111111111");
        usleep(100000);
    }
    ...
    stream_config_t audiostream_config_user1 = {.type = ACK_T, .ackstream_waittime_ms = 500, .is_encrypt = false};
    ...
    printf("user1 sendbuffer size is %d\n", mimc_rtc_get_sendbuffer_max_size(user1));

    sleep(3);

    while(times--) {
        char* tmpdata = (char*)malloc(size);
        const char* ctx = "dasda1232313132312323131323";
        mimc_rtc_send_data(user1, callid, (const char*)tmpdata, size, VID, RELAY_T, ctx, strlen(ctx), false, P1_T, 10);
        free(tmpdata);
        usleep(1000000 / qps);
    }
