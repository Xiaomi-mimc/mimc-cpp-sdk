...
#endif
string appAccount1 = "effi_MI1532";
string appAccount2 = "effi_MI1083";
string appAccount3 = "effi_MI1103";
...
public:
	...
	//@test
	void efficiencyTest() {
	    XMDLoggerWrapper::instance()->info("TEST_TIME_111");
		curl_global_init(CURL_GLOBAL_ALL);
		XMDLoggerWrapper::instance()->info("TEST_TIME_222");
	...
	void testEfficiency(User* user1, RtsEfficiencyHandler* callEventHandler1, User* user2, RtsEfficiencyHandler* callEventHandler2, User* user3, RtsEfficiencyHandler* callEventHandler3, int idx, int dataSize, map<int, RtsPerformanceData>& timeRecords) {
		...
		user1->sendRtsData(callId, sendData, AUDIO);
		for (int j = 0; j < TIME_OUT; j++) {
		...
		user2->sendRtsData(callId, sendData, AUDIO);
		for (int j = 0; j < TIME_OUT; j++) {
			if (callEventHandler3->getMsgSize(4) > 0) {
				break;
			}
...
int main(int argc, char **argv) {
    XMDLoggerWrapper::instance()->info("TEST_TIME_111333");
	RtsEfficiency testEff;
	testEff.efficiencyTest();
}
